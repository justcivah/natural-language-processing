...
for i in range(len(dataset_paths)):
    ...
    trainer = StudentTrainer(train_dataset, valid_dataset, student_output_paths[i], train_epochs=5, learning_rate=2e-5)
    trainer.train_student()

    test_dataset_a = dataset_paths[0] + datasets['test']
    test_dataset_b = dataset_paths[1] + datasets['test']

    model_path = os.path.abspath(student_output_paths[i] + 'best_model/')
