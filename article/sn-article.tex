...
In recent years, more and more cumbersome LLMs have come to life in the AI race led by OpenAI and joined by all other competitors and big techs like Google, Anthropic, and Meta, just to say some. These companies have created models with billions if not trillions of parameters \cite{abacha2025medec}, and are general purpose, which means they are capable of doing a wide range of tasks such as natural language understanding and generation, decision making, and reasoning \cite{ye2025llms4allreviewlargelanguage}.
...
For this study, both T5 and mT5 variants were tested in order to investigate whether a multilingual model has advantages on cross-lingual knowledge distillation, or if monolingual models with proper finetuning can also achieve similar performance on Italian translation tasks.
...
In total, 12 models were fine-tuned for 30 epochs on a machine provided by the MIPS Laboratory, equipped with an NVIDIA 3090 (24GB), an Intel i9-12900K, and 64GB of system RAM. All models were trained using FP16 precision, batches of size 16, a learning rate of 2e-5, and chrF++ as metric for evaluating the best model. For a more specific hyperparameter configuration, see code in \texttt{train\_student.py}. The training of all 12 models took about two days. 
...
Models fine-tuned on a single domain exhibit substantial performance degradation when evaluated on the other domain. For example, mT5-Base trained on Domain A achieves a chrF++ score of 71.1 on Domain A but only 45.9 on Domain B. This domain shift effect is consistent across all model architectures, with smaller models showing more severe degradation. Example translations for one sentence from each dataset are provided in Tables~\ref{tab:translation_a} and~\ref{tab:translation_b}. Table~\ref{tab:translation_a} shows translations of the sentence from Dataset A: \textit{"I've heard that some people sleep in bathtubs."}, while Table~\ref{tab:translation_b} shows translations of the sentence from Dataset B: \textit{"In 2003, for example, the murder rate in the 12 states without the death penalty was 4.1 for every 100,000 people."}
...
\begin{table}[!h]
	\centering
	\renewcommand{\arraystretch}{1.3}
	\begin{tabular}{m{0.2\linewidth} c m{0.25\linewidth} m{0.3\linewidth}}
		\hline
		...
		\hline
		T5-Efficient-Small & A & \textbf{Ho savo che persone sarebbe a scuola in settimana.} & In 2003, il tuo \`e stato a scuola \`e stato a scuola a scuola \`e stato a scuola a scuola \`e stato a tutti i miei sul tuo giorno a scuola \`e stato a tutti i giorni a scuola \`e \`e stato \`e stato \`e stato \`e stato \`e stato \`e stato \`e stato \`e stato \`e stato \`e stato \`e stato \`e stato \`e stato \`e stato \`e stato \`e stato \`e stato \`e stato \`e stato \\
		\hline
		T5-Efficient-Small & B & I risultati che alcuni persone saranno in acqua. & \textbf{In 2003, per esempio, la tua tua tua tua tua tua tua scuola di tuoi 12 paesi che l'infrastruttura era 4.1 per ogni 50,000 persone.} \\
		\hline
		T5-Efficient-Small & AB & \textbf{Ho che che i persone sul tuo tuo sul tuo tuo sul tuo sul tuo sul tuo sul tuo.} & \textbf{In 2003, per esempio, la rate di murdere nel 12 states non \`e stato 4.1 per il tuo tuo giorno.} \\
		\hline
		T5-Efficient-Base & A & \textbf{Ho sentito che qualcuno stanno dormire nel tubo.} & In 2003, per esperienza il polizio del 12 polizio \`e stato risultati a tutti i loro polizio a tutti i loro senza l'incidente delle Stati Uniti \`e stato 4.1 per tutti i loro polizio delle Stati Uniti che la risulta delle Stati l'\`e stato il il il il il il il anni anni anni anni anni anni a riso riso riso riso a tre tre tre anni. \\
		\hline
		T5-Efficient-Base & B & I hanno spieghi che alcuni persone sai in acqua. & \textbf{In 2003, per esempio, il tasso di homicide nei 12 stati con il punito di morte era il 4.1 per ogni milioni di persone.} \\
		\hline
		T5-Efficient-Base & AB & \textbf{Ho sentito che alcuni persone sa senza a scaff\`e.} & \textbf{Nel 2003, per esempio, la tassa di murdere nei 12 stati con il penalty di morte era 4.1 per ogni milioni di persone.} \\
		\hline
	\end{tabular}
	\vspace{4pt}
	\caption{Comparison of example translations from different non-multilingual models for test datasets A and B.}
	\label{tab:translation_a}
\end{table}
...
\begin{table}[!h]
	...
	\begin{tabular}{m{0.2\linewidth} c m{0.25\linewidth} m{0.3\linewidth}}
		...
		GPT-5 Mini & - & Ho sentito che alcune persone dormono nelle vasche da bagno. & Nel 2003, ad esempio, il tasso di omicidi nei 12 stati senza pena di morte era di 4,1 ogni 100.000 persone. \\
		\hline
		mT5-Small & A & \textbf{Ho sentito che molte persone dormono in bagni.} & In 2003, per quanto l'indice di morte in 12 stati senza la pena di morte \`e stata 4.1 per ogni 100.000 persone. \\
		\hline
		mT5-Small & B & Io sono ascoltati che alcuni persone dormono in bagni. & \textbf{Nel 2003, per esempio, il tasso di morte nei 12 stati senza la pena di morte era di 4.1 per ogni 100.000 persone.} \\
		\hline
		mT5-Small & AB & \textbf{Ho sentito che alcune persone dormono in bagni.} & \textbf{In 2003, per esempio, il tasso di morte nei 12 stati senza la penalit\`a di morte era 4.1 per ogni 100.000 persone.} \\
		\hline
		mT5-Base & A & \textbf{Ho sentito che alcune persone dormono nei bagni.} & In 2003, per exemple, il tasso di omicidio negli 12 Stati senza la pena di morte era 4.1 per cada 100.000 persone. \\
		\hline
		mT5-Base & B & Sono sentito che alcune persone dormono nei bagni. & \textbf{Nel 2003, per esempio, il tasso di omicidio nei 12 stati senza la pena di morte era di 4,1 per ogni 100.000 persone.} \\
		\hline
		mT5-Base & AB & \textbf{Ho sentito che alcune persone dormono nei bagni.} & \textbf{Nel 2003, per esempio, il tasso di omicidio nei 12 stati senza la pena di morte era di 4,1 per ogni 100.000 persone.} \\
		\hline
	\end{tabular}
	\vspace{4pt}
	\caption{Comparison of example translations from different multilingual models for test datasets A and B.}
	\label{tab:translation_b}
\end{table}
...
These results show that parameter count is the main indicator of translation quality, with mT5-Base constantly outperforming all other models across all domains. Furthermore, multilingual pretraining gives an advantage over monolingual counterparts, even though some monolingual models have achieved similar performances after finetuning, suggesting that multilingual is beneficial but not a strict necessity when sufficient task specific supervision is available.
